...
def register_disabled() -> None:
    raise AppError(
        "Legacy register endpoint is disabled. Use POST /onboarding/tenant to create tenant + admin.",
...
def login(payload: LoginRequest, session: Session = Depends(get_db_session)) -> LoginResponse:
    ...
    return LoginResponse(
        ...
        user=UserResponse(**user.to_dict(include_tenant=True, tenant=tenant)),
...
def create_tenant_with_admin(payload: OnboardingTenantRequest, session: Session = Depends(get_db_session)) -> OnboardingTenantResponse:
    ...
    existing_tenant = session.scalar(select(Tenant).where(Tenant.name.ilike(name)))
    if existing_tenant:
    ...
    tenant = Tenant(name=name, plan_status="ACTIVE")
    session.add(tenant)
    session.flush()

    admin_user = User(tenant_id=tenant.id, username=username, email=admin_email, role="ADMIN")
    admin_user.set_password(admin_password)
    session.add(admin_user)
    session.flush()

    log_audit(session, actor_user_id=admin_user.id, action="CREATE", entity_type="TENANT", entity_id=tenant.id, payload={"name": tenant.name, "onboarding": True})
    ...
    return OnboardingTenantResponse(
        ...
        tenant=TenantResponse(**tenant.to_dict()),
        owner_user=UserResponse(**admin_user.to_dict(include_tenant=False)),
        access_token=access_token,
        refresh_token=refresh_token,
    )
