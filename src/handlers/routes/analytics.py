...
def get_incidents_analytics(current_user: User = Depends(get_current_user), session: Session = Depends(get_db_session), hours: int = 24) -> dict:
    ...
    alerts = session.scalars(select(Alert).where(Alert.tenant_id == tenant_id, Alert.created_at >= time_threshold)).all()
    severity_count = {
...
def get_response_time_analytics(current_user: User = Depends(get_current_user), session: Session = Depends(get_db_session), days: int = 7) -> dict:
    ...
    resolved_alerts = session.scalars(select(Alert).where(Alert.tenant_id == tenant_id, Alert.status == "resolved", Alert.created_at >= time_threshold)).all()
    response_times = []
...
def get_vulnerability_distribution(current_user: User = Depends(get_current_user), session: Session = Depends(get_db_session)) -> dict:
    require_tenant_read_access(current_user)
    tenant_id = effective_tenant_id_of(current_user)
    vulnerabilities = session.scalars(select(Vulnerability).where(Vulnerability.tenant_id == tenant_id, Vulnerability.status == "open")).all()
...
def get_analytics_summary(current_user: User = Depends(get_current_user), session: Session = Depends(get_db_session)) -> dict:
    require_tenant_read_access(current_user)
    tenant_id = effective_tenant_id_of(current_user)
    active_alerts = session.query(Alert).filter_by(tenant_id=tenant_id, status="active").count()
    open_vulnerabilities = session.query(Vulnerability).filter_by(tenant_id=tenant_id, status="open").count()
    pending_tickets = count_tenant_tickets(tenant_id, status="PENDING")
    return {
