...
from src.persistence.models import Alert, Ticket, User, Vulnerability
...
def get_incidents_analytics(current_user: User = Depends(get_current_user), session: Session = Depends(get_db_session), hours: int = 24) -> dict:
    ...
    alerts = session.scalars(select(Alert).where(Alert.company_id == current_user.company_id, Alert.created_at >= time_threshold)).all()
    severity_count = {
...
def get_response_time_analytics(current_user: User = Depends(get_current_user), session: Session = Depends(get_db_session), days: int = 7) -> dict:
    ...
    resolved_alerts = session.scalars(select(Alert).where(Alert.company_id == current_user.company_id, Alert.status == "resolved", Alert.created_at >= time_threshold)).all()
    response_times = []
...
def get_vulnerability_distribution(current_user: User = Depends(get_current_user), session: Session = Depends(get_db_session)) -> dict:
    vulnerabilities = session.scalars(select(Vulnerability).where(Vulnerability.company_id == current_user.company_id, Vulnerability.status == "open")).all()
...
def get_analytics_summary(current_user: User = Depends(get_current_user), session: Session = Depends(get_db_session)) -> dict:
    active_alerts = session.query(Alert).filter_by(company_id=current_user.company_id, status="active").count()
    open_vulnerabilities = session.query(Vulnerability).filter_by(company_id=current_user.company_id, status="open").count()
    pending_tickets = session.query(Ticket).filter_by(company_id=current_user.company_id, status="PENDING").count()
    return {
