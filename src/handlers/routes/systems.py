...
def create_system(
    ...
    system = System(
        ...
        type=payload.get("type", "server"),
        status=payload.get("status", "online"),
        health_score=payload.get("health_score"),
        meta_info=payload.get("meta_info"),
    )
    session.add(system)
    log_audit(session, actor_user_id=current_user.id, action="CREATE", entity_type="SYSTEM", payload={"name": system.name})
    session.commit()
    return {"message": "System created successfully", "system": system.to_dict()}
